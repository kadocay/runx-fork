...
Next: none
Reason: Superseded by the hard OSS final-shape cutover. `@runxhq/core` and
`packages/core/**` are now deleted; cloud registry data/types moved to first-class
cloud/protocol packages and the Rust runtime owns install/client behavior. This
document is retained only as historical context for the earlier hosted-registry
transition and is no longer an active plan.
Blockers: none.
